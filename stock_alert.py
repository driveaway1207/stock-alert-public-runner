...
# 9）交易决策层：大周期负责是否值得跟踪，日线回踩承接负责是否正式推送，最终只推前3只。
# ===========================================================================
...
MODEL_VERSION = "V12.8.4多周期最大阳量K精确版"
...
# V12：一号员工正式报告默认只推前3只；后台候选池/跟踪池仍保留更多记录。
...
# V12.8.4：多周期最大阳量K高点窗口严格分层，避免周期/价格错位。
MAX_BULL_VOL_LOOKBACK_DAILY = int(os.environ.get("MAX_BULL_VOL_LOOKBACK_DAILY", "1000"))
MAX_BULL_VOL_LOOKBACK_WEEKLY = int(os.environ.get("MAX_BULL_VOL_LOOKBACK_WEEKLY", "500"))
MAX_BULL_VOL_LOOKBACK_MONTHLY = int(os.environ.get("MAX_BULL_VOL_LOOKBACK_MONTHLY", "100"))
MAX_BULL_VOL_LOOKBACK_QUARTERLY = int(os.environ.get("MAX_BULL_VOL_LOOKBACK_QUARTERLY", "50"))
...
def _find_valid_max_volume_bull_levels(period_df, tf_name, lookback=80):
    """
    V12.8.4：严格在指定周期和指定窗口内寻找“最大阳量K”。
    这里的高点/实底必须带周期、窗口和日期，不能跨周期混用。
    """
    empty = {
        "valid": False,
        "timeframe": tf_name,
    ...
    w = w.dropna(subset=["volume", "open", "high", "low", "close"]).reset_index(drop=True)
    if w.empty:
        return empty

    bull = w[w["close"] > w["open"]].copy()
    if bull.empty:
        return empty

    # 必须是该周期窗口内“阳K成交量最大”的有效阳K；阴K/小实体长影不参与。
    for _, row in bull.sort_values("volume", ascending=False).iterrows():
        ...
        return {
            ...
            "volume": float(vol),
            "reason": (
                f"【{tf_name}】最近{int(lookback or 0)}根K最大阳量K(date={date_text})："
                f"实底{floor:.2f}/实体中位{(floor + top) / 2:.2f}/实顶{top:.2f}/最高点{hi:.2f}"
            ),
        }
    return empty
...
def detect_multi_timeframe_key_structure(df, code=""):
    """
    V12.8.4 多周期最大阳量K关键位系统：
    先分周期，再在各自固定窗口内找最大阳量K，再看日线是否有效突破该周期高点。
    周期窗口：日线1000、周线500、月线100、季线50。报告必须写清周期/窗口/日期/实底/实顶/高点/日线触发状态。
    """
    empty = {
    ...
    contexts = []
    tf_defs = [
        ("日线", df.copy().reset_index(drop=True), MAX_BULL_VOL_LOOKBACK_DAILY, 1.0),
        ("周线", _resample_ohlcv(df, "W-FRI"), MAX_BULL_VOL_LOOKBACK_WEEKLY, 1.6),
        ("月线", _resample_ohlcv(df, "M"), MAX_BULL_VOL_LOOKBACK_MONTHLY, 2.4),
        ("季线", _resample_ohlcv(df, "Q"), MAX_BULL_VOL_LOOKBACK_QUARTERLY, 3.0),
    ]
    ...
    for tf_name, pdf, lookback, weight in tf_defs:
        ...
        body_mid = safe_float(level.get("body_mid", 0))
        body_top = safe_float(level.get("body_top", 0))
        pull = _count_dynamic_pullback_segments(pdf, floor)
        brk = _daily_break_quality_against_level(df, high)
        break_valid = bool(brk.get("valid", False))
        break_label = str(brk.get("label", ""))
        break_desc = str(brk.get("desc", ""))
        high_dist = (cur_close / high - 1) if high > 0 and cur_close > 0 else 0.0
        floor_dist = (cur_close / floor - 1) if floor > 0 and cur_close > 0 else 0.0

        usage = "历史结构背景"
        if break_valid and high_dist <= 0.25:
            usage = "日线有效突破观察位"
        elif high > 0 and abs(high_dist) <= 0.08:
            usage = "当前高点压力/触发观察位"
        elif floor > 0 and 0 <= floor_dist <= 0.12:
            usage = "当前实底承接观察位"
        elif high_dist > 0.40:
            usage = "已明显远离，仅作历史结构背景，不能作为当前追高加分"
        elif high_dist < -0.10:
            usage = "尚未突破，作为上方压力观察"

        floor_status_score = 0.0
        if floor > 0 and cur_close > 0:
            if cur_close >= floor and floor_dist <= 0.12:
                floor_status_score += 2.0 * weight
            elif -0.02 <= floor_dist < 0:
                floor_status_score += 1.0 * weight
            if pull.get("count", 0) >= 1 and floor_dist <= 0.18:
        ...
        # 日线有效突破该周期最大阳量K高点才允许明显加分；若已远离高点，不再给当前买点高分。
        if not break_valid and high_dist > 0.30:
            break_score = min(break_score, 0.0)
        if high_dist > 0.40:
            floor_status_score = min(floor_status_score, 1.0 * weight)
        ...
        contexts.append({
            ...
            "high": float(high),
            "volume": safe_float(level.get("volume", 0)),
            "current_to_high": float(high_dist),
            "current_to_floor": float(floor_dist),
            "usage": usage,
    ...
    resonance_count = sum(1 for x in contexts[1:] if safe_float(x.get("score", 0)) >= 3.0 and bool(x.get("break_valid", False)))
    resonance_bonus = min(4.0, resonance_count * 1.2)
    total_score = min(20.0, base_score + resonance_bonus)
    desc_parts = []
    for x in contexts[:4]:
        desc_parts.append(
            f"【{x['timeframe']}】最近{x['lookback']}根K最大阳量K(date={x['date']})："
            f"实底{x['floor']:.2f}/中位{x['body_mid']:.2f}/实顶{x['body_top']:.2f}/高点{x['high']:.2f}，"
            f"当前距高点{x['current_to_high']:.1%}，日线状态={x['break_label']}，用途={x['usage']}"
        )
    return {
        ...
        "multi_tf_levels_json": json.dumps(contexts[:8], ensure_ascii=False),
    }



def _find_valid_max_volume_bull_row(period_df, lookback=100):
    """V12.8.4：返回指定周期窗口内的最大阳量K及其位置，必须由调用方传入清晰周期窗口。"""
    ...
    if bull.empty:
        return None
    for idx, row in bull.sort_values("volume", ascending=False).iterrows():
        if _is_valid_max_volume_bull_candle(row):
            return idx, row, w
    return None


def _probe_k_quality(row, max_vol_bull_high, max_vol_bull_volume):
    """试盘K质量：创周期最大阳量K高点的新高，量价不能差。"""
    op = safe_float(row.get("open", 0))
    cl = safe_float(row.get("close", 0))
    hi = safe_float(row.get("high", 0))
    lo = safe_float(row.get("low", 0))
    vol = safe_float(row.get("volume", 0))
    if min(op, cl, hi, lo) <= 0 or max_vol_bull_high <= 0:
        return 0.0, "试盘K无效"
    rng = max(hi - lo, 1e-9)
    body = abs(cl - op)
    upper = hi - max(op, cl)
    close_pos = (cl - lo) / rng
    vol_ratio = vol / max_vol_bull_volume if max_vol_bull_volume > 0 else 0.0
    score = 0.0
    parts = []
    if hi >= max_vol_bull_high * 1.003:
        score += 2.0
        parts.append("高点突破最大阳量K高点")
    if vol_ratio >= 0.45:
        score += 1.2
        parts.append(f"量能/最大阳量K{vol_ratio:.2f}")
    if vol_ratio >= 0.65:
        score += 0.8
    if body >= rng * 0.35:
        score += 1.0
...
def detect_v124_probe_high_second_confirm_model(df):
    """
    V12.4 远期最大阳量K高点 + 高质量9号试盘高点 + 时间倍数 + 日线二次确认模型。
    ...
    for tf_name, pdf, lookback, weight in tf_defs:
        ...
        if found is None:
            continue
        max_vol_bull_idx, max_vol_bull_row, w = found
        max_vol_bull_high = safe_float(max_vol_bull_row.get("high", 0))
        max_vol_bull_vol = safe_float(max_vol_bull_row.get("volume", 0))
        if max_vol_bull_high <= 0 or max_vol_bull_vol <= 0 or max_vol_bull_idx >= len(w) - 8:
            continue
        after = w.iloc[max_vol_bull_idx + 1:].copy().reset_index(drop=False).rename(columns={"index":"orig_idx"})
        # 9号：最大阳量K高点之后创出更高高点的高质量试盘K，优先选择质量高且时间间隔合理的。
        probes = []
        for _, r in after.iterrows():
            orig_idx = int(r.get("orig_idx", -1))
            if orig_idx <= max_vol_bull_idx:
                continue
            hi = safe_float(r.get("high", 0))
            if hi < max_vol_bull_high * 1.003:
                continue
            q, qdesc = _probe_k_quality(r, max_vol_bull_high, max_vol_bull_vol)
            if q < 3.0:
                continue
            n = orig_idx - max_vol_bull_idx
            if n < 3:
                continue
        ...
        # 父级压力：最大阳量K之前更高的历史大凹口/大高点，若离红线过近则降级。
        before_max_vol_bull = w.iloc[:max_vol_bull_idx].copy()
        parent_pressure = 0.0
        if not before_max_vol_bull.empty:
            parent_pressure = safe_float(before_max_vol_bull["high"].max())
        ...
        false_cnt = 0
        for _, rr in w.iloc[max_vol_bull_idx + 1:].iterrows():
            if safe_float(rr.get("high", 0)) >= max_vol_bull_high * 1.003 and safe_float(rr.get("close", 0)) < max_vol_bull_high * 1.005:
                false_cnt += 1
        ...
        if valid and score >= 10 and parent_penalty > -5.0:
            ...
            stage = "突破小门但父级压力贴脸"
        max_vol_bull_date = str(max_vol_bull_row.get("date", ""))
        desc = (
            f"【{tf_name}】最近{lookback}根K最大阳量K(date={max_vol_bull_date})高点{max_vol_bull_high:.2f}，"
            f"试盘高点{probe_high:.2f}；试盘质量{q:.1f}({qdesc})；"
            f"N={n}，M={m}，{tlabel}；日线{brk.get('label','')}；{parent_label}"
        )
        cand = {
            "score_v124_probe_second_confirm": float(score),
            "v124_probe_stage": stage,
            "v124_probe_desc": desc,
            "v124_max_vol_bull_high": float(max_vol_bull_high),
...
def detect_v125_timing_window_model(df, v124_ctx=None):
    ...
    # 1）时间对称：优先引用V12.4最大阳量K高点/9号线模型输出，避免重复重算远期结构。
    time_score = 0.0
    time_desc = "无远期时间对称"
...
def calc_deep_rows(df, code):
    ...
    # ========================= V12.4：远期最大阳量K高点+9号试盘高点+时间倍数+日线二次确认 =========================
...
def process_stock_deep(row):
    ...
    for _, r in recent.iterrows():
        rows.append({
            ...
            "v124_max_vol_bull_high": float(r.get("v124_max_vol_bull_high", 0)) if pd.notna(r.get("v124_max_vol_bull_high", 0)) else 0,
...
def build_reason(s):
    ...
    if s.get("score_v124_probe_second_confirm", 0) > 0:
        reasons.append(
            f"V12.4远期最大阳量K高点/9号线二次确认：{s.get('score_v124_probe_second_confirm', 0):.1f}分，"
            f"阶段={s.get('v124_probe_stage', '')}，最大阳量K高点{s.get('v124_max_vol_bull_high', 0):.2f}，9号线{s.get('v124_probe_price', 0):.2f}，"
...
def build_reason_v12(s):
    ...
    if safe_float(s.get("score_v124_probe_second_confirm", 0)) >= 6:
        watch.append("远期最大阳量K高点/9号线二次确认模型：" + str(s.get("v124_probe_desc", "")))
    ...
    if safe_float(s.get("v124_parent_distance", 0)) > 0 and safe_float(s.get("v124_parent_distance", 0)) < 0.08:
        problems.append("9号线/最大阳量K高点上方仍有更大父级凹口压力贴脸，突破小门不等于打穿大门")
...
def build_message(final_signals, dates, stock_count=0, kline_success=0, kline_fail=0, deep_count=0):
    lines = []
    lines.append("📊 <b>一号员工V12.8.2风险硬过滤+可操作性三选报告</b>")
    ...
    lines.append(f"V12.8.3：财务硬雷区优先；日线追高/量能未确认/右侧平台均量不足的票禁止进入正式三选；正式推送前{RESULT_LIMIT}只。")
    lines.append("V12.6规则：风险硬过滤→基础轻扫→候选深算→多周期关键位→重心平量/1000日时窗/底部充分率→统一突破/回踩/量能/活跃度→同源合并→前三正式推送。")
...
# ========================= V12.8.3 风险硬过滤 + 日线不追高硬门槛 =========================
def v1282_candidate_block_reason(row):
    """
    正式三选硬门槛：
    财务/审计/治理雷区优先级高于技术形态；
    无可操作性、明显追高、离防守位过远的票，不得进入正式三选。
    """
    reasons = []
    ...
    if "ST" in name_upper or "退" in name:
        reasons.append("ST/退市风险名称")
    if "-UW" in name_upper or "-U" in name_upper or "未盈利" in name:
        reasons.append("未盈利/UW属性，禁止进入正式三选")

    try:
        risk = evaluate_regulatory_risk(code, name)
        risk_flags = "；".join(risk.get("flags", [])) + ("；" + str(risk.get("note", "")) if risk.get("note") else "")
    except Exception:
        risk = {"hard_exclude": False, "penalty": 0, "flags": [], "note": ""}
        risk_flags = ""

    risk_text = risk_flags + "；" + str(row.get("regulatory_risk_flags", "")) + "；" + str(row.get("risk_flags", "")) + "；" + str(row.get("risk_note", ""))
    hard_risk_keywords = [
        "审计报告保留意见", "保留意见", "无法表示意见", "否定意见", "非标审计", "非标准审计",
        "立案调查", "信披违规", "资金占用", "违规担保", "退市风险", "债务违约",
        "财报亏损", "扣非亏损", "连续亏损", "亏损扩大", "高负债率", "Z值风险",
        "控股股东高质押", "高质押", "诉讼风险", "重大诉讼", "营收陡降", "子公司风险",
        "安全分较差", "安全分低", "财务亏损",
    ]
    if bool(row.get("risk_hard_exclude", False)) or bool(risk.get("hard_exclude", False)):
        reasons.append("命中重大监管/财务硬雷区")
    if safe_float(row.get("score_regulatory_risk", 0)) <= -35:
        reasons.append("雷区扣分过重")
    for kw in hard_risk_keywords:
        if kw and kw in risk_text:
            reasons.append(f"重大风险：{kw}")
            break
    ...
    near_p = safe_float(row.get("near_pressure_dist", 0))
    mid_p = safe_float(row.get("mid_pressure_dist", 0))
    distance_to_key = safe_float(row.get("distance_to_key", row.get("distance_to_key_base", 0)))
    ...
    chase_count = int(safe_float(row.get("chase_risk_count", 0)))
    pullback_score = safe_float(row.get("score_v12_pullback_entry", 0))
    trade_quality = safe_float(row.get("score_trade_quality", 0))
    trade_priority = safe_float(row.get("trade_priority_score", 0))
    structure_score = safe_float(row.get("score_structure_core", 0))
    monthly_score = safe_float(row.get("score_monthly_cycle", 0))
    multi_tf_score = safe_float(row.get("score_multi_tf_key_structure", 0))
    timing_score = safe_float(row.get("score_v125_timing_window", 0))
    step_score = safe_float(row.get("score_v125_step_platform_lift", 0))
    step_vol_ratio = safe_float(row.get("v125_step_volume_ratio", 0))
    step_price_lift = safe_float(row.get("v125_step_price_lift", 0))
    step_break_trigger = bool(row.get("v125_step_break_trigger", False))
    timing_trigger = bool(row.get("v125_timing_trigger", False))
    v125_vol_stability = safe_float(row.get("v125_volume_stability_score", 0))
    v125_flat_ratio = safe_float(row.get("v125_flat_volume_ratio", 0))
    v126_vol_flat_score = safe_float(row.get("v126_volume_flat_score", 0))

    try:
        limit_threshold = get_limit_threshold(code)
    except Exception:
        limit_threshold = 9.3
    is_limit_up = bool(row.get("limit_up", False)) or pct >= limit_threshold
    strong_attack = is_limit_up or pct >= 7.0 or entity_pct >= 7.0
    daily_chase = pct >= 4.5 or entity_pct >= 4.5 or break_rate >= 0.04
    very_extended = (bias20 >= 0.12 and bias60 >= 0.14) or bias20 >= 0.18 or distance_to_key >= 0.09 or defense_dist >= 0.10
    # V12.8.3：月线/季线在中位只能说明大周期可观察，不能抵消日线追高风险。
    # 日线已经涨起来、没有回踩承接、且量能没有完成右侧确认时，正式三选直接拦截。
    right_platform_volume_not_confirmed = (step_price_lift > 0.03 and 0 < step_vol_ratio < 0.95 and not step_break_trigger)
    pre_launch_volume_unstable = (v125_vol_stability < 1.2 and v125_flat_ratio < 0.55) or (v126_vol_flat_score < 1.0 and v125_flat_ratio < 0.50)

    if daily_chase and pullback_score < 6.0 and (right_platform_volume_not_confirmed or pre_launch_volume_unstable):
        reasons.append("日线已涨/接近追高，但右侧平台量能或启动前量能稳定性未确认")
    if daily_chase and (monthly_score + multi_tf_score >= 10) and pullback_score < 5.0:
        reasons.append("月线/季线可观察但日线买点未到，不能用大周期中位掩盖日线追高")
    if pct >= 3.5 and right_platform_volume_not_confirmed and pullback_score < 7.0:
        reasons.append("右侧平台均量低于左侧起涨平台，放量确认不足")

    if strong_attack and very_extended and pullback_score < 5.0:
        reasons.append("单日强攻后乖离/离关键位过远，缺少回踩承接，禁止追高")
    if pct >= 5.0 and long_pos >= 0.70 and pullback_score < 6.0:
        reasons.append("中高位上涨后无舒服回踩买点")
    if break_rate >= 0.08 and distance_to_key >= 0.07:
        reasons.append("突破已远离结构位，交易防守过远")
    if defense_dist >= 0.10 and rr > 0 and rr < 1.50:
        reasons.append("止损距离偏大且风险收益比不足")
    if near_p > 0 and near_p < 0.05 and pct >= 4.0:
        reasons.append("强攻贴近近端压力，缺乏上方空间")
    if mid_p > 0 and mid_p < 0.035 and pct >= 4.0:
        reasons.append("强攻贴近中层压力")
    if chase_count >= 3:
        reasons.append("追高风险项过多")
    if timing_score >= 10 and structure_score < 6 and pullback_score < 5 and step_score < 6:
        reasons.append("仅有时间窗口/弱修复，缺少可交易触发")
    if trade_priority > 0 and trade_priority < 55:
        reasons.append("交易优先级不足")
    if trade_quality < -3 and pullback_score < 5:
        reasons.append("交易质量为负且无回踩确认")

    pool = str(row.get("candidate_pool", "优先候选池"))
    if pool != "优先候选池":
        reasons.append(f"非优先候选池：{pool}")
...
def v1282_is_formal_candidate(row):
    return v1282_candidate_block_reason(row) == ""
# ========================= V12.8.3 风险硬过滤 + 日线不追高硬门槛结束 =========================
...
def main():
    ...
    try:
        ...
        print("V12.7稳定提速版：评分逻辑保持V12.6不变；新增旧缓存兜底、BaoStock失败转AkShare补拉、连续失败快速熔断、失败清单诊断；风险硬过滤→基础轻扫→候选深算→前3只正式推送。")
        ...
        blocked_formal_count = 0
        for r in deep_rows:
            if float(r.get("total_score", 0)) < FINAL_SCORE_THRESHOLD:
                continue

            pool = str(r.get("candidate_pool", "优先候选池"))
            block_reason = v1282_candidate_block_reason(r)
            if block_reason:
                r["v1282_block_reason"] = block_reason
                r["candidate_pool"] = "高风险/无操作性观察池"
                if SAVE_STRONG_WATCH_POOL == "1":
                    strong_watch_pool.append(r)
                blocked_formal_count += 1
                continue
        ...
        try:
            print(f"V12.8.3正式三选拦截数量：{blocked_formal_count} 只（财务硬雷区/无可操作性/追高风险）")
        except Exception:
            pass
