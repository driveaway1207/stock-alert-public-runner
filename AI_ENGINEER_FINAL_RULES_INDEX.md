...
- 所有员工中文名字、编号、身份定位、职责边界、workflow 显示名、报告标题和主脚本身份默认全部锁定；只有用户明确要求修改时才允许改。AI 工程师自己理解、发挥、统一命名、架构升级式改名都视为未授权身份漂移，按猴子代码风险处理。
- 零号员工必须审计所有员工身份漂移，而不只是审计代码语法、关键文件和生产链路。
...
- 零号员工负责代码审计、猴子代码、生产链路风险和员工身份漂移风险；六号员工负责文档归档和成功经验沉淀。
