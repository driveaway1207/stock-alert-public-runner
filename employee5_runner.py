...
MAX_STOCKS = int(os.getenv("EMPLOYEE5_MAX_STOCKS", "120"))
AK_TIMEOUT_SECONDS = int(os.getenv("EMPLOYEE5_AK_TIMEOUT_SECONDS", "18"))
REQUEST_SLEEP = float(os.getenv("EMPLOYEE5_REQUEST_SLEEP", "0.15"))
...
def board_limit(code: str, name: str) -> Tuple[str, float]:
    code, name = ss(code), ss(name).upper()
    if "ST" in name:
        return "ST", 5.0
...
def fetch_limit_pool(target_date: str) -> pd.DataFrame:
    parts = []
    try:
        with timeout_guard(AK_TIMEOUT_SECONDS, "stock_zt_pool_em"):
            parts.append(normalize_pool(ak.stock_zt_pool_em(date=target_date), "zt_pool"))
    except Exception as e:
        print("zt_pool failed:", e)
    try:
        with timeout_guard(AK_TIMEOUT_SECONDS, "stock_zh_a_spot_em"):
            parts.append(normalize_pool(ak.stock_zh_a_spot_em(), "spot"))
    except Exception as e:
        print("spot failed:", e)
    parts = [p for p in parts if p is not None and not p.empty]
    if not parts:
        return pd.DataFrame(columns=["code", "name", "pct_chg", "close", "board", "limit_pct"])
    pool = pd.concat(parts, ignore_index=True).drop_duplicates("code")
    boards = pool.apply(lambda r: board_limit(r["code"], r["name"]), axis=1)
    pool["board"] = [x[0] for x in boards]
    pool["limit_pct"] = [x[1] for x in boards]
    pool = pool[pool.apply(lambda r: is_limit_up(sf(r["pct_chg"]), sf(r["limit_pct"])), axis=1)]
    return pool.sort_values(["limit_pct", "pct_chg"], ascending=False).head(MAX_STOCKS).reset_index(drop=True)
...
def analyze_stock(row: pd.Series, target_date: str) -> Dict[str, Any]:
    ...
    if df.empty or len(df) < 30:
        return {"code": code, "name": name, "board": board, "error": "历史K线不足"}
    df = add_ma(df)
    ma = ma_report(df)
    ...
    reasons = []
    if czt >= 2:
        reasons.append(f"{czt}连板")
    else:
        reasons.append("首板样本")
    reasons.extend(tags[:8])
    reasons.append(f"20日量比{vol.get('vol_ratio_20')}")
    return {"code": code, "name": name, "board": board, "limit_pct": limit_pct, "close": round(sf(last.get("close")), 3), "pct_chg": round(sf(last.get("pct_chg")), 2), "consecutive_limit_up": czt, "grade": grade, "ma_lines": ma, "volume": vol, "tags": tags, "reasons": reasons}

def build_outputs(target_date: str, pool: pd.DataFrame, results: List[Dict[str, Any]]) -> Tuple[str, Dict[str, Any]]:
    ...
    lines = ["🧬【五号员工-涨停板归因】", f"日期：{target_date}", f"涨停样本：{len(pool)}只", f"等级分布：{json.dumps(grade_count, ensure_ascii=False)}"]
    if top_tags:
        lines.append("\n高频强势结构：")
    ...
    data = {"target_date": target_date, "limit_up_count": len(pool), "grade_count": grade_count, "board_count": board_count, "top_tags": top_tags, "results": results}
    return "\n".join(lines), data
...
def main() -> None:
    ...
    pool = fetch_limit_pool(target_date)
    if pool.empty:
        msg = f"🧬【五号员工-涨停板归因】\n日期：{target_date}\n未识别到涨停样本。"
        (REPORT_DIR / "limit_up_research_report.md").write_text(msg, encoding="utf-8")
        (REPORT_DIR / "limit_up_research_report.json").write_text(json.dumps({"target_date": target_date, "limit_up_count": 0, "results": []}, ensure_ascii=False, indent=2), encoding="utf-8")
        print(msg)
        send_tg(msg)
        return
    results = []
    for i, (_, row) in enumerate(pool.iterrows(), 1):
        print(f"分析 {i}/{len(pool)}：{row.get('name')}({row.get('code')})")
        try:
            results.append(analyze_stock(row, target_date))
        except Exception as e:
            results.append({"code": row.get("code"), "name": row.get("name"), "error": str(e)})
    text, data = build_outputs(target_date, pool, results)
