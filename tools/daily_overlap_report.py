...
def env_with_target(extra: Dict[str, str]) -> Dict[str, str]:
    target = os.getenv("OVERLAP_TARGET_DATE") or default_trade_date()
    env = os.environ.copy()
    env.update({
        "PYTHONUNBUFFERED": "1",
...
def kline_metrics(code: str) -> Dict[str, Any]:
    ...
    out: Dict[str, Any] = {"latest_close": round(latest, 2)}
    for days in (5, 20, 60):
        if len(close) > days:
...
def build_html(payload: Dict[str, Any]) -> str:
    ...
    for r in payload["stocks"]:
        ...
        <article class="stock-card" data-code="{html.escape(r['code'])}" data-name="{html.escape(r['name'])}" data-industry="{html.escape(r['industry'])}" data-hot="{html.escape(r['hot_level'])}">
          ...
          <dl><div><dt>行业</dt><dd>{html.escape(r['industry'])}</dd></div><div><dt>5日</dt><dd>{fmt(m.get('pct_5d'))}%</dd></div><div><dt>20日</dt><dd>{fmt(m.get('pct_20d'))}%</dd></div><div><dt>量比</dt><dd>{fmt(m.get('volume_ratio'))}</dd></div></dl>
...
def send_telegram(url: str, payload: Dict[str, Any]) -> bool:
    ...
    if not token or not chat or requests is None:
        return False
    lines = ["股票共振池已生成", f"时间：{payload['generated_at_bj']}", f"重合股票：{len(payload['stocks'])} 只", f"网页：{url}"]
...
def main() -> int:
    ...
    payload = {
        ...
        "target_date": os.getenv("OVERLAP_TARGET_DATE") or default_trade_date(),
        "stocks": stocks,
    ...
    if os.getenv("OVERLAP_SEND_TELEGRAM", "1") == "1":
        ...
        REPORT_JSON.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    print(json.dumps({"ok": True, "stocks": len(stocks), "html": str(REPORT_HTML), "json": str(REPORT_JSON)}, ensure_ascii=False))
    return 0
