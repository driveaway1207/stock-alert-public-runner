...
def local_points(df: pd.DataFrame, col: str, window: int = 2, kind: str = "high") -> List[Tuple[int, float]]:
    vals = [sf(x) for x in df[col].tolist()]
    pts: List[Tuple[int, float]] = []
    if len(vals) < window * 2 + 1:
        return pts
    for i in range(window, len(vals)-window):
        seg = vals[i-window:i+window+1]
        if kind == "high" and vals[i] == max(seg) and vals[i] > 0:
            pts.append((i, vals[i]))
        if kind == "low" and vals[i] == min(seg) and vals[i] > 0:
            pts.append((i, vals[i]))
    return pts
...
def p(no: int, name: str, hit: bool, evidence: str, score: float = 0.0) -> Dict[str, Any]:
    return {"no": no, "name": name, "hit": bool(hit), "score": rd(score), "evidence": evidence}


def d(no: int, cat: str, name: str, hit: bool, evidence: str, score: float = 0.0) -> Dict[str, Any]:
    return {"no": no, "category": cat, "name": name, "hit": bool(hit), "score": rd(score), "evidence": evidence}


def detect_archetypes(df: pd.DataFrame) -> List[Dict[str, Any]]:
    daily = base.add_indicators(df.copy())
    monthly = aggregate_period(daily, "ME")
    work = monthly if len(monthly) >= 24 else daily
    close = sf(daily.iloc[-1].close)
    high_250 = base.prev_high(daily, 250)
    pos_250 = base.range_position(daily, 250)
    ...
    highs = local_points(work, "high", 2, "high")
    hs_hit = False
    hs_ev = "三峰不足或形态不清晰"
    ...
    result.append(p(1, "大级别头肩/三峰供给释放后再启动", hs_hit, hs_ev, 3 if hs_hit else 0))
    ...
    if len(daily) >= 260:
        ...
        result.append(p(2, "长周期下跌后底部磨底吸收", deep_dd >= 35 and width <= 80 and cv <= 1.25, f"前高后深跌={deep_dd}% ，近180日宽度={width}% ，量能CV={cv}", 2.5))
    result.append(p(3, "左侧年线级前高突破", bool(high_250 and close >= high_250), f"250日高点={rd(high_250)}，当前={rd(close)}，250日位置={pos_250}", 2.8 if high_250 and close >= high_250 else 0))

    if len(daily) >= 180:
        v_old = sf(daily.iloc[-180:-90].volume.mean())
        v_mid = sf(daily.iloc[-90:-30].volume.mean())
        v_now = sf(daily.iloc[-20:].volume.mean())
        result.append(p(4, "量能枯竭后恢复", v_old > 0 and v_mid < v_old * 0.75 and v_now > v_mid * 1.2, f"量能均值 old/mid/now={rd(v_old,0)}/{rd(v_mid,0)}/{rd(v_now,0)}", 2.3))

    h100 = base.prev_high(daily, 100)
    recent_big = sum(1 for x in daily.iloc[-40:].pct_chg.tolist() if sf(x) >= 6)
    result.append(p(5, "凹口附近巨震/大阳攻击后蓄势", bool(h100 and abs(pct(close, h100)) <= 8 and recent_big >= 1), f"100日凹口线={rd(h100)}，距当前={rd(pct(close,h100)) if h100 else 0}% ，近40日大阳次数={recent_big}", 2.2))

    h60, l60 = base.prev_high(daily, 60), base.prev_low(daily, 60)
    width60 = rd((h60 / l60 - 1) * 100) if l60 else 0
    result.append(p(6, "中期箱体吸收后突破", bool(width60 <= 35 and close >= h60 > 0), f"60日箱体宽度={width60}% ，是否突破上沿={bool(close >= h60 > 0)}", 2.4))

    if len(daily) >= 120:
        a, b = daily.iloc[-120:-70], daily.iloc[-65:-15]
        price_up = sf(b.close.mean()) > sf(a.close.mean()) * 1.06
        vol_up = sf(b.volume.mean()) > sf(a.volume.mean()) * 1.05
        result.append(p(7, "台阶平台价格/量能中枢抬升", price_up and vol_up, f"价格中枢抬升={price_up}，量能中枢抬升={vol_up}", 2.0))
    ...
    result.append(p(8, "BOLL/BBI缩口后中轨修复", bw > 0 and bwr <= .3 and close >= sf(daily.iloc[-1].get("boll_mid")), f"BOLL带宽分位={rd(bwr*100,1)}%，中轨={rd(daily.iloc[-1].get('boll_mid'))}", 2.0))
    return result
...
def build_dimensions(code: str, name: str, board: str, hist: pd.DataFrame) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    vr5, vr20, vr60 = base.vol_ratio(df, 5), base.vol_ratio(df, 20), base.vol_ratio(df, 60)
    ...
    react = base.reaction_count(df, near_core)
    vbp = base.vbp_pressure(df)
    anchor = base.find_vol_anchor(df)
    sweep = base.sweep_memory(df, near_core)
    mvk = base.max_volume_k(df)
    archetypes = detect_archetypes(df)
    dims: List[Dict[str, Any]] = []
    no = 1
    def add(cat: str, name_: str, hit: bool, ev: str, score: float = 0.0) -> None:
        nonlocal no
        dims.append(d(no, cat, name_, hit, ev, score))
        no += 1
    ...
    add("时间路径", "250日/年线区间位置", (base.range_position(df,250) or 0) <= .65, f"250日/年线区间分位={base.range_position(df,250)}", 1.2)
    add("核心线", "20日结构高点突破", close >= h20 > 0, f"20日高点={rd(h20)}，当前={rd(close)}", 1.1)
    ...
    add("量能资金", "健康放量区间", 1.4 <= vr20 <= 4.5, f"5/20/60日量比={vr5}/{vr20}/{vr60}", 1.4)
    ...
    if len(df) >= 90:
        ...
        add("量能资金", "平台均量中枢抬升", sf(right.volume.mean()) > sf(left.volume.mean()) * 1.05, f"前/后平台均量={rd(left.volume.mean(),0)}/{rd(right.volume.mean(),0)}", 1.3)
    sub = df.iloc[-45:-1]
    upv, dnv = sf(sub[sub.close >= sub.open].volume.mean()), sf(sub[sub.close < sub.open].volume.mean())
    add("量能资金", "阳量压阴量", div(upv, dnv) >= 1.15, f"45日阳阴量比={rd(div(upv,dnv))}", 1.2)
    add("触发K线", "涨停K实体质量", cm["body_ratio"] >= .55 and cm["close_pos"] >= .8, f"K线={cm}", 1.8)
    add("触发K线", "跳空/缺口强攻", cm["gap_pct"] >= 1.5, f"跳空幅度={cm['gap_pct']}%", 1.1)
    ...
    add("零件战法", "二阶低点斜率抬升", len(df) >= 80 and lin_slope(df.iloc[-35:-1].low.tolist()) > 0, f"近段低点斜率={rd(lin_slope(df.iloc[-35:-1].low.tolist()),4) if len(df>=80 else 0}", 1.0)
    ...
    defense_candidates = [x for x in [ma5, sf(df.iloc[-2].low), sf(df.iloc[-1].get("boll_mid")), near_core*.98 if near_core else 0] if x and x < close]
    defense = max(defense_candidates) if defense_candidates else 0
    risk = pct(close, defense) if defense else 99
    reward = pct(max(h250, close*1.1), close)
    add("华尔街框架", "RR/结构防守", defense > 0 and risk <= 12, f"防守={rd(defense)}，风险={rd(risk)}%，空间={rd(reward)}%", 1.3)
    add("华尔街框架", "共性统计价值", sum(1 for x in dims if x.get("hit")) >= 12, f"已命中维度数={sum(1 for x in dims if x.get('hit'))}", 1.0)
...
def summarize_hits(deep: List[Dict[str, Any]], key: str) -> Dict[str, int]:
    ...
    for item in deep:
        for x in item.get(key, []):
            if x.get("hit"):
                if key == "dimensions":
                    k = f"D{int(x['no']):02d} {x['category']}｜{x['name']}"
                else:
                    k = f"AR{int(x['no']):02d} {x['name']}"
...
def main() -> None:
    ...
    base_deep = data.get("deep_samples", [])[:3]
    deep: List[Dict[str, Any]] = []
    hist_source_count: Dict[str, int] = {}
    for item in base_deep:
        ...
        one = dict(item)
        one["hist_source"] = source
        one["dimensions"] = dims
        one["archetypes"] = archetypes
        deep.append(one)
    arch_count = summarize_hits(deep, "archetypes")
    dim_count = summarize_hits(deep, "dimensions")
    lines = [
        "🧬【五号员工-结构原型归因增强报告】",
        f"日期：{target_date}",
        f"结构增强耗时：{base.fmt_seconds(time.time()-start)}",
        "口径：先识别大级别结构原型，再统计可转化维度；不把数据源、周期换算、空话当维度。",
        f"历史K线来源：{json.dumps(hist_source_count, ensure_ascii=False)}",
        "",
        "一、结构原型共振统计：",
    ]
    if arch_count:
        for k, v in sorted(arch_count.items(), key=lambda x: x[1], reverse=True):
            lines.append(f"- {k}：{v}只")
    else:
        lines.append("- 本次3只深度样本未形成明显共同结构原型，需要保留未识别结论，不能硬套。")
    lines += ["", "二、可转化维度共振统计："]
    if dim_count:
        for k, v in sorted(dim_count.items(), key=lambda x: x[1], reverse=True)[:24]:
            lines.append(f"- {k}：{v}只")
    else:
        lines.append("- 无明显共振维度。")
    lines += ["", f"三、深度样本结构归因："]
