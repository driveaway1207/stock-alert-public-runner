...
def has_marker(path: Path, mk: str) -> bool:
    return path.exists() and mk in path.read_text(encoding="utf-8", errors="ignore")
...
def list_recent_commit_shas(event: Dict[str, Any]) -> List[str]:
    ...
    六号员工不再高频 schedule 抢 runner；每次由 workflow_run / daily / manual 触发后，
    回扫最近 40 个 commit。已经写过 marker 的提交不会重复记录。
    """
    shas: List[str] = []
    ...
    return list(dict.fromkeys([s for s in shas if s]))[:50]
...
def is_self_archive_commit(message: str) -> bool:
    return message.strip().startswith(AUTO_COMMIT_PREFIX)
...
def classify_path(path: str) -> str:
    ...
    if p.startswith(".github/workflows/"):
        return "workflow"
    if re.search(r"employee0|employee_0|零号", p):
        return "employee0"
    if re.search(r"employee1|employee_1|一号", p) or p.endswith("stock_alert.py"):
        return "employee1"
...
def infer_purpose(message: str, files: List[str]) -> str:
    ...
    if "workflow" in m or ".github/workflows" in joined:
        return "自动触发/运行链路：workflow、并发锁、触发频率或依赖安装。"
    if "employee" in joined or "员工" in message or "employee" in m:
        return "员工体系更新：涉及员工脚本、员工手册、workflow或报告规范。"
    if "战法" in message or "模型" in message or any(x in m for x in ["strategy", "model"]):
        return "战法/模型规则更新：需要登记，未复盘前不得写成最终成功。"
...
def change_entry(s: Dict[str, Any]) -> str:
    return "\n".join([
        f"## {now_text()}｜Commit `{s['short_sha']}`",
        "",
...
def success_entry(s: Dict[str, Any]) -> str:
    return "\n".join([
        f"## {now_text()}｜自动沉淀候选：`{s['short_sha']}`",
        "",
        "- 状态：已落地代码/文档经验候选。若后续被用户否定，必须更正或移出成功经验库。",
        f"- commit message：{s['message'].splitlines()[0] if s['message'] else '无'}",
        f"- 经验归纳：{s['purpose']}",
        f"- 影响范围：{json.dumps(s['classes'], ensure_ascii=False)}",
        "- 对应路径：",
        md_list(s["files"][:30]),
    ])


def should_enter_success_ledger(s: Dict[str, Any]) -> bool:
    if not s["files"]:
        return False
    classes = s["classes"]
    return bool("workflow" in classes or "employee0" in classes or "employee5" in classes or "employee6_docs" in classes or "global_docs" in classes or any(f.endswith(".md") for f in s["files"]))


def write_workflow_entry(wf: Dict[str, Any]) -> bool:
    if not wf:
        return False
    wf_key = f"workflow-{wf.get('workflow_name')}-{wf.get('run_number')}-{wf.get('conclusion')}"
    mk = marker("workflow", wf_key)
    entry = "\n".join([
...
def main() -> None:
    ...
    for sha in list_recent_commit_shas(event):
        c = fetch_commit(sha)
        if not c:
            continue
        s = build_commit_summary(c)
        if is_self_archive_commit(s["message"]):
            continue
        summaries.append(s)

    changed = False
    changed = write_workflow_entry(wf) or changed
    for s in summaries:
        change_mk = marker("change", s["sha"][:12])
        changed = insert_after_title(CHANGE_LOG, CHANGE_HEADER, change_entry(s), change_mk) or changed
        if should_enter_success_ledger(s):
            ledger_mk = marker("ledger", s["sha"][:12])
            changed = insert_after_title(SUCCESS_LEDGER, SUCCESS_HEADER, success_entry(s), ledger_mk) or changed
